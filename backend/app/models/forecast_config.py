...
class ForecastConfig(Base):
    ...
    # Revenue — per-month JSON dicts keyed "1"–"12"
    small_job_counts = Column(JSON, nullable=False, default=dict)           # {"1": 5, "2": 4, ...}
    small_job_avg_value = Column(BigInteger, nullable=False, default=0)      # legacy scalar (kept for data safety)
    small_job_avg_value_monthly = Column(JSON, nullable=False, default=dict) # {"1": 80000, ...} cents

    medium_job_counts = Column(JSON, nullable=False, default=dict)
    medium_job_avg_value = Column(BigInteger, nullable=False, default=0)
    medium_job_avg_value_monthly = Column(JSON, nullable=False, default=dict)

    large_job_counts = Column(JSON, nullable=False, default=dict)
    large_job_avg_value = Column(BigInteger, nullable=False, default=0)
    large_job_avg_value_monthly = Column(JSON, nullable=False, default=dict)

    # Payroll
    cost_per_pay_run = Column(BigInteger, nullable=False, default=0)           # legacy scalar (kept for fallback)
    cost_per_pay_run_monthly = Column(JSON, nullable=False, default=dict)     # {"1": 250000, ...} cents per month
    pay_runs_per_month = Column(JSON, nullable=False, default=dict)           # {"1": 2, "2": 2, ...}
    payroll_one_off = Column(JSON, nullable=False, default=dict)              # {"3": 25000, ...} cents
    ...
    depreciation_monthly = Column(JSON, nullable=False, default=dict)    # {"1": 25000, ...}

    # Other income/expense — per-month cents, can be negative
    other_income_expense_monthly = Column(JSON, nullable=False, default=dict)

    # Cash flow drivers — per-month days (integer, not cents)
    dso_monthly = Column(JSON, nullable=False, default=dict)  # {"1": 30, ...} days sales outstanding
    dio_monthly = Column(JSON, nullable=False, default=dict)  # {"1": 0, ...} days inventory outstanding
    dpo_monthly = Column(JSON, nullable=False, default=dict)  # {"1": 30, ...} days payable outstanding
    ...
    capex_monthly = Column(JSON, nullable=False, default=dict)
    other_current_assets_change_monthly = Column(JSON, nullable=False, default=dict)
    current_debt_change_monthly = Column(JSON, nullable=False, default=dict)
    long_term_debt_change_monthly = Column(JSON, nullable=False, default=dict)
